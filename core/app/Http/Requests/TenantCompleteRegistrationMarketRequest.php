...
class TenantCompleteRegistrationMarketRequest extends FormRequest {
    ...
    public function rules(): array {
        return [
            'market.sectors' => ['required', 'array', 'min:1'],
            'market.sectors.*' => ['exists:sectors,id'],
            'market.org_name_en' => ['required', 'string', 'max:50'],
            'market.org_name_ar' => ['required', 'string', 'max:50'],
    ...
    protected function prepareForValidation() {
        if (empty($this->input('domain.domain'))) {
            $this->merge([
                'domain' => [
                    'domain' => $this->generateUniqueSubdomain($this->input('market.org_name_en'))
                ],
            ]);
        }
    }
