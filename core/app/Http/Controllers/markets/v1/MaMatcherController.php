...
use App\Http\Requests\Matcher\CheckAvailabilityMarket;
...
class MaMatcherController extends Controller {
    ...
    public function check_availability(CheckAvailabilityMarket $request) {
        $validated = $request->validated();
        $veServiceIds = ServiceVendor::where('vendor_id',$validated['vendor_id'])->whereIn('ve_service_id',$validated['service_ids'])->where('status',1)->pluck('ve_service_id')->toArray();
        $veProvidersIds = VendorProvider::where('vendor_id',$validated['vendor_id'])->where('status',1)->pluck('provider_id')->toArray();
        $available = [];
        $vendor = MaVendor::findOrFail($validated['vendor_id']);
        $tenant = Tenant::find($vendor->tenant_id);
        foreach ($veServiceIds as $vId){
            $available[$vId] = $tenant->run(function () use ($vId,$veProvidersIds) {
                $service = VeServices::find($vId);
                $matchingProviders = Providers::where('status', 1)
                    ->whereIn('id',$veProvidersIds)
                    ->whereHas('services', function ($query) use ($vId) {
                        $query->where('service_id', $vId);
                    })->pluck('id')->toArray();
                $providerCount = 1;
        ...
        return $this->getSuccessResponse('success', $available);
    }
