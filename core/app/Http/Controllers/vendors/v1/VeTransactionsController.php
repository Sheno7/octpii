...
class VeTransactionsController extends Controller {
    use ResponseTrait;

    public function index(GetTransactions $request) {
        try {
            $data = Transaction::with(['paymentMethod', 'package', 'provider', 'booking', 'expense', 'procurement'])
        ...
        if ($branch_id) {
            $query->where(function ($query) use ($branch_id) {
                $query->whereHas('booking', function ($q) use ($branch_id) {
                    ...
                    $q->where('branch_id', $branch_id);
                })->orWhereHas('provider', function ($q) use ($branch_id) {
                    $q->where('branch_id', $branch_id);
                });
            });
        }
    ...
    public function update(UpdateTransaction $request) {
        $inputs = $request->validated();
        $transaction = Transaction::findOrFail($inputs['id']);
    ...
    private function getCustomerTotalBookingSpent($customerId) {
        try {
            $total = Transaction::join('booking', 'booking.id', '=', 'transaction.booking_id')
    ...
    private function getCutomerTotalPackageSpent($customerId) {
        try {
            $total = Transaction::join('package', 'package.id', '=', 'transaction.package_id')
        ...
        try {
            ...
            foreach ($customer_outstanding['bookings'] as $booking) {
                ...
                $tmp = [
                    ...
                    'date' => $inputs['date'],
                    'provider_id' => BookingProvider::where('booking_id', $booking->id)->value('provider_id'),
