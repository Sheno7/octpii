...
use App\Http\Requests\TenantCompleteRegistrationRequest;
...
class AuthTenantController extends Controller {
    ...
    public function completeRegistration(TenantCompleteRegistrationRequest $request) {
        $inputs = $request->validated();
        try {
            DB::beginTransaction();
            $user = $request->user();
            $vendor = $user->vendor()->create($inputs['vendor']);
            $vendor->status = 1;
            $vendor->save();
            $user->status = 1;
            $user->save();
            $vendor->sectors()->sync($inputs['vendor']['sectors']);
            // $plan = Plan::find($inputs['vendor']['plan_id']);
            // $vendor->subscribeTo($plan);
            DB::commit();
            config(['tenancy.migration_parameters.--path' => [database_path('migrations/_vendor')]]);
            $tenant = Tenant::create([
                'name' => $request->input('domain.domain'),
                'user_id' => $user->id,
                'vendor_id' => $vendor->id,
                'tenancy_db_name' => $vendor->id . ".api",
                'status' => 1
            ]);
