...
class MaCustomersController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query = Customers::select(
                ...
                'countries.code',
                'customers.status',
                DB::raw('case when customers.status = 1 then \'Active\' else \'Inactive\' end as status')
            )
            ...
            foreach ($data as $customer) {
                $transaction = new MaTransactionsController();
    ...
    public function show(int $id)
    {
        try {
            $data = Customers::select(
                ...
                ->first();
            $transaction = new MaTransactionsController();
            $data->total_spent = $transaction->getCustomerTotalSpent($id);
            $data->total_booking = Booking::where('customer_id', $id)->count();
            $data->address = Address::where('owner_id', $id)->where('owner_type', 1)->get();
            $data->favorite_vendors = MaVendor::
            join('user_vendor', 'user_vendor.vendor_id', '=', 'vendors.id')
                ->where('user_vendor.user_id', $data->user_id)->get();
            return $this->getSuccessResponse('success', $data);
        } catch (\Throwable $th) {
            Logger::error($th->getMessage());
            return $this->getErrorResponse('error', $th->getMessage());
    ...
    {
        try {
            $media = (new MaMediaController())->getMediaByModelId('xrayimage', $id);
            $combinedData = [
                "type" => "XRAy",
                "hasfile" => true,
                "value" => [
    ...
    public function listAddress()
    {
        try {
            $data = DB::table('address')
                ->select('areas.id', 'areas.title_en as area_title', 'cities.id as city_id', 'cities.title_en as city_title', 'countries.title_en as country_title', 'countries.id as countries.id', 'address.id', 'address.owner_id', 'address.owner_type', 'address.area_id', 'address.location_name', 'address.street_name', 'address.building_number', 'address.floor_number', 'address.unit_number', 'address.lat', 'address.lng', 'address.notes')
                ->join('areas', 'address.area_id', '=', 'areas.id')
                ->leftJoin('cities', 'areas.city_id', '=', 'cities.id')
                ->leftJoin('countries', 'cities.country_id', '=', 'countries.id')
                ->where('address.owner_id', auth()->id())
    ...
    public function updateStatus(Request $request, Customers $customers)
    {
        try {
            $request->validate(['status' => 'required|in:0,1']);
            $customers->update(['status' => $request->status]);
            return $this->getSuccessResponse('success');
        } catch (\Throwable $th) {
            Logger::error($th->getMessage());
            return $this->getErrorResponse('error', $th->getMessage());
